...
"""MCP tools available only when the CLI agent is hosted inside FreeCAD.
...
async def gui_documents_list(args):
    try:
        ...
        return _ok(gui_thread.run_sync(_read))
...
async def gui_new_document(args):
    try:
        ...
        return _ok({"document": gui_thread.run_sync(_create, timeout=60.0)})
    except Exception as exc:
        return _err(str(exc), traceback=traceback.format_exc(limit=4))
...
async def gui_open_document(args):
    try:
        ...
        return _ok({"document": gui_thread.run_sync(_open, timeout=60.0)})
    except Exception as exc:
        return _err(str(exc), traceback=traceback.format_exc(limit=4))
...
async def gui_set_active_document(args):
    try:
        ...
        return _ok({"document": gui_thread.run_sync(_activate, timeout=30.0)})
    except Exception as exc:
        return _err(str(exc), traceback=traceback.format_exc(limit=4))
...
async def gui_reload_active_document(args):
    try:
        ...
        return _ok(gui_thread.run_sync(_reload, timeout=60.0))
    except Exception as exc:
        return _err(str(exc), traceback=traceback.format_exc(limit=4))
