...
def coverage_rows(doc_path: str) -> list[dict]:
    ...
    try:
        ...
        return []
    di = data.get("design_intent") or {}
    # Prefer the immutable snapshot taken on first write — the model has
    # been observed to descope features by editing spec_from_drawing
    # after the fact (drop "32×R0.09" once it can't satisfy that count).
    spec = di.get("spec_from_drawing_initial") or di.get("spec_from_drawing") or ""
